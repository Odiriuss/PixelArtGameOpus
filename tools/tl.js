// usage: node tl.js steps.json -- like run.js, but eval failures are reported (with the page errors) instead of aborting
...
(async () => {
  ...
  for (const s of steps) {
    try {
      if (s.url) await page.goto(pageUrl('hourglass_testlevel.html', s.url), { waitUntil: 'load' });
    ...
    } catch (e) { console.log('STEP FAILED: ' + JSON.stringify(s).slice(0, 120) + '\n  ' + e.message.split('\n')[0]); }
  }
