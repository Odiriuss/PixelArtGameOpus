...
// Sprites are lit by the same terms as the city's tiles, for this hour: the sky, the sun (unless a building is in
// the way), every lamp near them that is switched on, neon by its flicker. Inside a building, by its own lamps.
// Dynamic light on the baked city is reconstructed from the depth buffer, as in the test level.
...
const DYN = [];                                              // this frame's dynamic lights {x, y, z, r, k, cone?, map?}
const NEAR = [];
// the lights that may reach a point: the city's (outdoors) or a floor's (inside), and this frame's flashes
function lightsNear(x, y, rad, F) {
  NEAR.length = 0;
  if (F) { for (const L of F.lights) if (Math.hypot(L.x - x, L.y - y) < L.r + rad) NEAR.push(L); }
  else {
  ...
  for (const L of DYN) {                                     // a cone (a headlight, a flashlight) only reaches what is in front of it
    const dx = x - L.x, dy = y - L.y, d = Math.hypot(dx, dy); if (d >= L.r + rad) continue;
    if (L.cone && d > 0.3 && (dx * L.dx + dy * L.dy) / d < L.cos) continue;
    NEAR.push(L);
  }
  return NEAR;
}
const SPR_LIFT = 0.7;                                        // figures read a little brighter than the ground they stand on
const TINT = { map: null, v: 0 };
function lampK(L, P) { if (!L.id) return L.k; if (!LAMP_ON[L.id]) return 0; if (L.tag && !FAST_ON[L.tag]) return 0; return L.k * (L.kind === 'int' ? 1 : P.lamp); }
// warm light at a point for a sprite; sun: 0..1, how much of the sun the thing is standing in
function litAt(list, x, y, z, P, sun) {
  let warm = P.amb + SPR_LIFT + P.sun * sun * 0.8; TINT.map = null; TINT.v = 0;
  for (let l = 0; l < list.length; l++) {
    const L = list[l], dx = L.x - x, dy = L.y - y, dz = L.z - z, d2 = dx * dx + dy * dy + dz * dz;
    if (d2 >= L.r * L.r) continue;
    const k = lampK(L, P); if (k <= 0) continue;
    const d = Math.sqrt(d2);
    let v = 1 - d / L.r; v = v * v * k * clamp(0.5 + (dx + dy) / (d + 1e-6) * 0.5, 0.25, 1);
    if (L.map) { if (v > TINT.v) { TINT.v = v; TINT.map = L.map; } } else warm += v;
  }
  return warm;
}
function sunOn(x, y, z, F) { return F ? 0 : (sunShadow(x, y, z + 1.0) ? 0 : 1); }
...
function vehEmissive(V, code) {
  ...
  if (code === CODE_SIGN) { if (V.model === 'police') return V.siren ? ((tick >> 3) & 1 ? C.RED : C.CRIM) : -1; return V.signOn ? C.PALEY : -1; }
  return -1;
}
function reflectVehicle(V, cx, cy) {
  ...
  for (let k = 0; k < fr.n; k++) {
    ...
    if ((m & MAT_GLOSS) && WEATHER.wet < 0.45) continue;
    if ((m & MAT_PUDDLE) && !(m & MAT_WATER) && WEATHER.wet < 0.15) continue;
...
function drawVehicle(V, cx, cy) {
  const fr = modelFrame(V.M, V.a), [bx, by] = vehScreen(V, cx, cy), base = V.x + V.y, lut = V.lut;
  const list = lightsNear(V.x, V.y, V.M.hl + 1, null).slice(), sink = V.z < -0.05, dmg = V.dmgLook || 0, flash = V.flash > 0, P = LP.out;
  const sun = V.kin ? sunOn(V.x, V.y, 1.5, null) : sunOn(V.x, V.y, 0.4, null);
  ...
  for (let k = 0; k < fr.n; k++) {
    ...
    if (col < 0) {
      ...
      if (cell !== lastCell) { lastCell = cell; const wx = (fr.x[k] / 16 + dd) * 0.5, wy = (dd - fr.x[k] / 16) * 0.5; warm = litAt(list, V.x + wx, V.y + wy, V.z + wz, P, sun) - 0.35; tmap = TINT.map; tv = TINT.v; }
      const b = bay(x, y), st = lightSteps(warm, 0.5 + (b - 0.5) * 0.45);
      if (code >= 200 && code < 220) {
        const base2 = code >= 210 ? 210 : 200, lv = code - base2 - dent + st;
        if (lv >= 0 && lv <= 4) col = lut[base2 + lv];
        else if (lv > 4) { col = lut[base2 + 4]; for (let q = 4; q < lv; q++) col = LIT[col]; }
        else { col = lut[base2]; for (let q = lv; q < 0; q++) col = SHD[col]; }
...
function drawPersonXray(a, cx, cy) {
  const f = actorFrame(a), flip = (a.dir === 1 || a.dir === 3), sx0 = Math.round(isoX(a.x, a.y)) - cx - SPR_AX, sy0 = Math.round(isoY(a.x, a.y, a.z || 0)) - cy - SPR_BY, depth = a.x + a.y + 0.15;
...
// ------------------------------------------------------------------ people: the shared renderer, given the lights of the hour
const CITY_ROOM = { lights: [], ambient: 0, spriteAmb: 0, rb: null };
const PROXY = [];
function drawPerson(a, cx, cy) {
  const F = a.F || null, P = F ? LP.ins : LP.out, list = lightsNear(a.x, a.y, 1, F), out = CITY_ROOM.lights;
  out.length = 0;
  for (let k = 0; k < list.length; k++) {
    const L = list[k], kk = lampK(L, P); if (kk <= 0) continue;
    const q = PROXY[k] || (PROXY[k] = {}); q.x = L.x; q.y = L.y; q.z = L.z; q.r = L.r; q.k = kk; q.map = L.map || null; q.tag = 0; out.push(q);
  }
  CITY_ROOM.ambient = P.amb + SPR_LIFT + P.sun * sunOn(a.x, a.y, a.z || 0, F) * 0.8 - 0.25;
  room = CITY_ROOM;
  drawActor(a, cx, cy);
}
// is this person where the player can see them: out in the street, or on the floor of the building whose roof is off
function personShown(p) {
  if (!p.visible || p.inCar) return false;
  if (!p.B) return true;
  return LIFT.b === p.B && LIFT.k > 0.5 && p.B.floors[LIFT.f] === p.F;
}
