...
const NAV = { x0: 0, y0: 0, gw: 80, gh: 80, cell: 0.5, g: new Uint8Array(6400), built: -999, cover: [], cx: 0, cy: 0 };
const NAVCARS = [];
...
function buildNav(cx, cy) {
  ...
  NAVCARS.length = 0; for (const V of VEH) if (!V.gone && Math.abs(V.x - cx) < 27 && Math.abs(V.y - cy) < 27) NAVCARS.push(V);
  for (let j = 0; j < NAV.gh; j++) for (let i = 0; i < NAV.gw; i++) {
    const x = NAV.x0 + (i + 0.5) * NAV.cell, y = NAV.y0 + (j + 0.5) * NAV.cell;
    NAV.g[j * NAV.gw + i] = blockedAt(x, y, 0.32) || vehicleAt(x, y, 0.32) ? 1 : 0;
  }
  NAV.built = tick;
  buildCover();
}
...
function coverScore(p, c, fx, fy, fV) {
  ...
  for (const q of PEOPLE) if (q !== p && q.team === 'goons' && q.alive && Math.hypot(q.x - c.x, q.y - c.y) < 1.1) return 1e9;
  if (lineClear(fx, fy, 1.3, c.x, c.y, 0.75, fV)) return 1e9;                      // Frank would see him crouched
  const peek = lineClear(c.x, c.y, 1.4, fx, fy, 1.1, null, fV);
